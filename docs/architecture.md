...
[浏览器]
   ...
   └─ HTTP → 单个 Uvicorn / FastAPI 容器（8000）
                    ├─ 单实例 MySQL 8
                    ├─ 单实例 Redis 7.2
                    ├─ AES-256-GCM 加密后的本地证据目录
                    ├─ LocalKMS（环境变量主密钥，仅开发/测试）
                    ├─ Mock CAS（可切换 Real CAS 3.0 Provider）
                    └─ Prometheus `/metrics` + 结构化标准输出日志
```

Docker Compose 会同时启动 MinIO，但当前
`backend/app/services/storage_service.py` 仍把加密后的证据写入
`EVIDENCE_UPLOAD_DIR`，尚未调用 MinIO/S3 API。演示时可以介绍 MinIO 是已经
准备好的对象存储基础设施，但不能声称当前证据文件已经实际写入 MinIO。

当前环境也没有部署 Nginx、TLS 证书、MySQL 主从、Redis Sentinel、
Grafana 或 ELK/Loki。它们属于下述生产目标，而不是本地演示已经运行的组件。
...
对象存储落地时应把 `storage_service` 改为 S3 客户端，上传加密后的字节并
保留现有哈希、密钥版本和权限检查。生产 KMS 还需实现当前占位的 Vault 或
AWS KMS Provider，确保主密钥不与密文位于同一主机或同一配置文件。
